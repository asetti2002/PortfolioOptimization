...
def run_simulation(model, env):
    ...
    while not done:
        ...
        # Assuming 'indices' key gives you the current date info.
...
def main():
    ...
    for algo_name, (date_list, portfolio_values) in results.items():
        plt.plot(date_list, portfolio_values, marker='o',markersize=.05, label=algo_name)
    if plot_ewp:
    ...
    save_path = os.path.join(save_folder, "cumulative_portfolio_value_for_RL_agents.png")
